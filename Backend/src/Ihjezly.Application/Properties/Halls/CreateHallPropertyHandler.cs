...
internal sealed class CreateHallPropertyHandler<THall, TDetails>
    : ICommandHandler<CreateHallPropertyCommand<THall, TDetails>, Guid>
    where THall : HallProperty<TDetails>, new()
{
    private readonly CreatePropertyHandler<THall, TDetails> _handler;
    private readonly INotificationRepository _notificationRepository;
    private readonly IUnitOfWork _unitOfWork;
    private readonly ISubscriptionRepository _subscriptionRepository;

    public CreateHallPropertyHandler(
        CreatePropertyHandler<THall, TDetails> handler,
    ...
    public async Task<Result<Guid>> Handle(CreateHallPropertyCommand<THall, TDetails> request, CancellationToken cancellationToken)
    {
        ...
        // Create the property
        var command = new CreatePropertyCommand<THall, TDetails>(
            Title: request.Title,
        ...
        if (result.IsSuccess)
        {
            ...
            var notification = Notification.Create(
                ...
                request.IsAd
                    ? $"تم إنشاء إعلان قاعة عقارك '{request.Title}' بنجاح."
                    : $"تم إنشاء قاعة عقارك '{request.Title}' بنجاح."
            );
