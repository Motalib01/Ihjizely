...
public class RegisterUserCommandHandler : ICommandHandler<RegisterUserCommand, UserDto>
{
    ...
    public async Task<Result<UserDto>> Handle(RegisterUserCommand request, CancellationToken cancellationToken)
    {
        ...
        {
            UserRole.Client => Client.Create(request.FirstName, request.LastName, request.PhoneNumber, hashedPassword, false),
            UserRole.BusinessOwner => BusinessOwner.Create(request.FirstName, request.LastName, request.PhoneNumber, hashedPassword, false),
            UserRole.Admin => Admin.Create(request.FirstName, request.LastName, request.PhoneNumber, hashedPassword, false),
